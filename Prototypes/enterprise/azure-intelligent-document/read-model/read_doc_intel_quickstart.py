# Notes : https://github.com/Azure-Samples/document-intelligence-code-samples/blob/main/Python(v4.0)/Read_model/README.md
# coding: utf-8
...
def analyze_read():
    ...
    # Key-value pairs
    markdown_content += "## Key-value pairs\n\n"
    # for kv_pair in result.key_value_pairs:
    #     if kv_pair.key and kv_pair.value:
    #         markdown_content += f"**Key**: `{kv_pair.key.content}`\n**Value**: `{kv_pair.value.content}`\n\n"
    #     else:
    #         markdown_content += f"**Key**: `{kv_pair.key.content}`\n**Value**: `None`\n\n"
    ...
    # Languages
    markdown_content += "## Languages\n\n"
    for language in result.languages:
        markdown_content += f"**Language**: {language.locale}\n**Confidence**: {language.confidence}\n\n"

    # Save to markdown file
    with open("document_analysis_result.md", "w") as file:
        file.write(markdown_content)

    print("Results saved to document_analysis_result.md")
    # [END analyze_read]
