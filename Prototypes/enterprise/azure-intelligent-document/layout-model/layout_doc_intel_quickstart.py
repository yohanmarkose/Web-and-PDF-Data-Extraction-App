...
def analyze_layout():
    ...
    if result.tables:
        for table_idx, table in enumerate(result.tables):
            markdown_content += f"Table # {table_idx} has {table.row_count} rows and {table.column_count} columns\n"
            if table.bounding_regions:
                for region in table.bounding_regions:
                    markdown_content += f"Table # {table_idx} location on page: {region.page_number} is {region.polygon}\n"
            # Cells
            for cell in table.cells:
                markdown_content += f"...Cell[{cell.row_index}][{cell.column_index}] has text '{cell.content}'\n"
                if cell.bounding_regions:
                    for region in cell.bounding_regions:
                        markdown_content += f"...content on page {region.page_number} is within bounding polygon '{region.polygon}'\n"
    else:
        markdown_content += "Document does not contain table content\n\n"

    # Figures
    markdown_content += "## Figures\n\n"
    if result.figures:
        for figures_idx, figures in enumerate(result.figures):
            markdown_content += f"Figure # {figures_idx} has the following spans: {figures.spans}\n"
            for region in figures.bounding_regions:
                markdown_content += f"Figure # {figures_idx} location on page: {region.page_number} is within bounding polygon '{region.polygon}'\n"

    # Save to markdown file
    with open("layout_analysis_result.md", "w" , encoding="utf-8") as file:
        file.write(markdown_content)

    print("Results saved to layout_analysis_result.md")
    # [END extract_layout]
